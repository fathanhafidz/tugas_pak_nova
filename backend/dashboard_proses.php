<?php
include '../config/koneksi.php';
error_reporting(E_ALL);
ini_set('display_errors', 1);
header('Content-Type: application/json');

// ==========================
// Summary Cards
// ==========================
$total_barang = $koneksi->query("SELECT COUNT(*) as total FROM barang")->fetch_assoc()['total'];
$total_kategori = $koneksi->query("SELECT COUNT(*) as total FROM kategori")->fetch_assoc()['total'];
$total_supplier = $koneksi->query("SELECT COUNT(*) as total FROM supplier")->fetch_assoc()['total'];

$barang_masuk_bulan = $koneksi->query("
    SELECT COALESCE(SUM(jumlah),0) as total 
    FROM barang_masuk 
    WHERE MONTH(waktu_masuk)=MONTH(CURDATE()) AND YEAR(waktu_masuk)=YEAR(CURDATE())
")->fetch_assoc()['total'];

$barang_keluar_bulan = $koneksi->query("
    SELECT COALESCE(SUM(jumlah),0) as total 
    FROM barang_keluar_detail bkd
    JOIN barang_keluar bk ON bk.id_keluar=bkd.id_keluar
    WHERE MONTH(bk.waktu_keluar)=MONTH(CURDATE()) AND YEAR(bk.waktu_keluar)=YEAR(CURDATE())
")->fetch_assoc()['total'];

$laba_bulan = $koneksi->query("
    SELECT COALESCE(SUM((bkd.harga_jual_satuan - bkd.harga_beli_satuan) * bkd.jumlah),0) as laba
    FROM barang_keluar_detail bkd
    JOIN barang_keluar bk ON bk.id_keluar=bkd.id_keluar
    WHERE MONTH(bk.waktu_keluar)=MONTH(CURDATE()) AND YEAR(bk.waktu_keluar)=YEAR(CURDATE())
")->fetch_assoc()['laba'];

// ==========================
// Grafik Barang Masuk vs Keluar per bulan
// ==========================
$grafik = [];
for ($i=1; $i<=12; $i++) {
    $masuk = $koneksi->query("
        SELECT COALESCE(SUM(jumlah),0) as total 
        FROM barang_masuk 
        WHERE MONTH(waktu_masuk)=$i AND YEAR(waktu_masuk)=YEAR(CURDATE())
    ")->fetch_assoc()['total'];

    $keluar = $koneksi->query("
        SELECT COALESCE(SUM(bkd.jumlah),0) as total 
        FROM barang_keluar_detail bkd
        JOIN barang_keluar bk ON bk.id_keluar=bkd.id_keluar
        WHERE MONTH(bk.waktu_keluar)=$i AND YEAR(bk.waktu_keluar)=YEAR(CURDATE())
    ")->fetch_assoc()['total'];

    $grafik[] = [
        "bulan" => date("F", mktime(0,0,0,$i,1)),
        "masuk" => $masuk,
        "keluar" => $keluar
    ];
}

// ==========================
// Grafik Laba per bulan
// ==========================
$grafik_laba = [];
for ($i=1; $i<=12; $i++) {
    $laba = $koneksi->query("
        SELECT COALESCE(SUM((bkd.harga_jual_satuan - bkd.harga_beli_satuan) * bkd.jumlah),0) as laba 
        FROM barang_keluar_detail bkd 
        JOIN barang_keluar bk ON bk.id_keluar=bkd.id_keluar 
        WHERE MONTH(bk.waktu_keluar)=$i AND YEAR(bk.waktu_keluar)=YEAR(CURDATE())
    ")->fetch_assoc()['laba'];

    $grafik_laba[] = [
        "bulan" => date("F", mktime(0,0,0,$i,1)),
        "laba" => $laba
    ];
}

// ==========================
// Barang Masuk Terbaru
// ==========================
$masuk_terbaru = [];
$qMT = $koneksi->query("
    SELECT bm.waktu_masuk, b.nama_barang, bm.jumlah 
    FROM barang_masuk bm 
    JOIN barang b ON b.id_barang=bm.id_barang
    ORDER BY bm.waktu_masuk DESC
    LIMIT 5
");
while ($row = $qMT->fetch_assoc()) {
    $masuk_terbaru[] = $row;
}

// ==========================
// Barang Keluar Terbaru
// ==========================
$keluar_terbaru = [];
$qKT = $koneksi->query("
    SELECT bk.waktu_keluar, b.nama_barang, bkd.jumlah
    FROM barang_keluar bk
    JOIN barang_keluar_detail bkd ON bkd.id_keluar = bk.id_keluar
    JOIN barang b ON b.id_barang = bkd.id_barang
    ORDER BY bk.waktu_keluar DESC
    LIMIT 5
");
while ($row = $qKT->fetch_assoc()) {
    $keluar_terbaru[] = $row;
}

// ==========================
// Activity Log Terbaru
// ==========================
$log_terbaru = [];
$qLT = $koneksi->query("
    SELECT al.waktu, u.username, al.aktivitas 
    FROM activity_log al 
    LEFT JOIN users u ON u.id_users=al.id_users
    ORDER BY al.waktu DESC 
    LIMIT 5
");
while ($row = $qLT->fetch_assoc()) {
    $log_terbaru[] = $row;
}

// ==========================
// Output JSON
// ==========================
echo json_encode([
    "summary" => [
        "total_barang" => $total_barang,
        "total_kategori" => $total_kategori,
        "total_supplier" => $total_supplier,
        "barang_masuk_bulan" => $barang_masuk_bulan,
        "barang_keluar_bulan" => $barang_keluar_bulan,
        "laba_bulan" => $laba_bulan
    ],
    "grafik" => $grafik,
    "grafik_laba" => $grafik_laba,
    "masuk_terbaru" => $masuk_terbaru,
    "keluar_terbaru" => $keluar_terbaru,
    "log_terbaru" => $log_terbaru
]);
?>
